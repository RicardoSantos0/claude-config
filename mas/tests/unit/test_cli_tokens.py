...
def _write_call(db, project_id, prompt=100, completion=50):
    append_event(
...
class TestQueryTokenUsage:
    """Unit tests for db.query_token_usage()."""
    ...
    def test_counts_call(self, tmp_db):
        from core.db import query_token_usage
        _write_call(tmp_db, "proj-live", prompt=100, completion=50)
        result = query_token_usage("proj-live", db_path=tmp_db)
        assert result["total"] == 150
        assert result["calls"] == 1

    def test_multiple_calls(self, tmp_db):
        from core.db import query_token_usage
        _write_call(tmp_db, "proj-mix", prompt=200, completion=100)
        _write_call(tmp_db, "proj-mix", prompt=0, completion=0)
        _write_call(tmp_db, "proj-mix", prompt=300, completion=150)
    ...
    def test_scoped_by_project(self, tmp_db):
        from core.db import query_token_usage
        _write_call(tmp_db, "proj-A", prompt=100, completion=50)
        _write_call(tmp_db, "proj-B", prompt=999, completion=999)
...
class TestCliTokens:
    ...
    def test_tokens_output_format(self, runner, tmp_path, monkeypatch):
        ...
        init_db(tmp_db)
        _write_call(tmp_db, "proj-tok-test", prompt=100, completion=50)
