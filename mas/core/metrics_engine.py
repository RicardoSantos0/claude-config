...
Weighting: equal for v1. All scored metrics averaged for overall.
Exemplary threshold: agent score > 90 → flag as exemplary.
...
class MetricsEngine:
    ...
    def score_scope_adherence(
        ...
        """
        Score based on planned vs actual task outcomes.
        100 = all tasks completed, no over-effort, no failures.
        Deductions:
          - Each blocked/failed task: -10 pts
          - Each over-effort task: -5 pts
        Minimum: 0.
        """
    ...
    def score_documentation_completeness(
        ...
        """
        Checklist of required documents. Score = present/total * 100.
        Required: clarified_spec.yaml, product_plan.yaml, execution_plan.yaml
        Recommended: shared_state snapshot, evaluation_report
        """
        required = [
        ...
        # Weight: required = 0.8, recommended = 0.2
    ...
    def score_phase_efficiency(
        ...
        """
        Measure efficiency as handoffs per phase.
        Ideal: 2 handoffs per phase (Master→Agent + Agent→Master).
        Penalty: each extra handoff above ideal per phase counts as -5 pts.
        Score starts at 100.
        """
        if not handoff_history:
            return MetricResult(
                ...
                findings="No handoff history to analyze",
            )

        # Count handoffs per phase
        phase_counts: dict = {}
        ...
        total_extra = sum(max(0, count - 2) for count in phase_counts.values())
        deduction = total_extra * 5
        score = max(0.0, 100.0 - deduction)

        findings_parts = [
            f"{phase}: {count} handoffs" + (" ✓" if count <= 2 else f" (−{(count-2)*5}pts)")
        ...
        return MetricResult(
            metric="phase_efficiency",
            score=score,
            evidence=f"{len(handoff_history)} total handoffs across {total_phases} phases",
            findings="; ".join(findings_parts),
            exemplary=score > EXEMPLARY_THRESHOLD,
        )
    ...
    def score_decision_quality(
        ...
        """
        Rubric (from expert answers):
          0-25:   Undocumented or contradicting
          26-50:  Documented without rationale
          51-75:  Documented with rationale
          76-100: Documented with rationale, alternatives, traceability

        Heuristic: check decision records for rationale/alternatives fields.
        If no decisions: 50 (neutral, not penalized).
        """
        if not decision_log:
            return MetricResult(
                ...
                findings="No decision log to evaluate",
            )

        total = len(decision_log)
        has_rationale = sum(
            1 for d in decision_log
            if d.get("rationale") or d.get("reasoning")
        )
        has_alternatives = sum(
            1 for d in decision_log
            if d.get("alternatives_considered") or d.get("alternatives")
        )
        has_traceability = sum(
            1 for d in decision_log
            if d.get("related_to") or d.get("traceability")
        )

        # Base: 51 (documented), + bonuses for quality
        base = 51.0
        rationale_bonus = (has_rationale / total) * 15.0
        alternatives_bonus = (has_alternatives / total) * 17.0
        traceability_bonus = (has_traceability / total) * 17.0
        score = min(100.0, base + rationale_bonus + alternatives_bonus + traceability_bonus)

        return MetricResult(
            metric="decision_quality",
            score=score,
            evidence=f"{total} decisions: {has_rationale} with rationale, {has_alternatives} with alternatives",
            findings=(
                f"Base: documented ({base:.0f}pts). "
                f"Rationale bonus: +{rationale_bonus:.1f}. "
                f"Alternatives bonus: +{alternatives_bonus:.1f}. "
                f"Traceability bonus: +{traceability_bonus:.1f}."
            ),
            exemplary=score > EXEMPLARY_THRESHOLD,
        )
    ...
    def score_task_completion_rate(
        ...
        If agent was never assigned any tasks: score = 100 (N/A, no penalty).
        """
    ...
    def score_handoff_quality(
        ...
        A handoff that is accepted immediately = first-try acceptance.
        Score = accepted_first_try / total_outgoing * 100.
        If no outgoing handoffs: 100.
        """
        outgoing = [
            h for h in handoff_history
            if h.get("from_agent") == agent_id
        ]
        if not outgoing:
            return MetricResult(
        ...
        # In v1, all accepted handoffs are first-try (no re-submission tracking yet)
        # Count handoffs with status == "accepted"
    ...
    def score_boundary_adherence(
        ...
        Also returns raw violation count.
        """
        agent_violations = [
            v for v in governance_violations
        ...
        findings = (
            f"{count} boundary violation(s) recorded."
            if count > 0
            else "No boundary violations."
        )
    ...
    def evaluate_project(
        ...
        """
        Compute all project-level metrics.
        Returns list[MetricResult].
        """
        ...
        # Count acceptance criteria from product plan (if plan exists)
        ...
        if plan_path.exists():
            ...
            # v1 heuristic: if all must-have tasks completed → all AC passed
            must_have_tasks = [t for t in tasks if t["status"] in ("completed",)]
        ...
        for m in metrics:
            if m.score < 60.0:
                recommendations.append(
                    f"Improve {m.metric}: {m.findings}"
                )
        if overall > EXEMPLARY_THRESHOLD:
            recommendations.append(
                "Consider promoting to T0_core or using as reference implementation"
            )
    ...
    def produce_report(
        ...
        agents_to_evaluate: list of agent_ids active in this project.
        """
        now = datetime.now(timezone.utc).isoformat()
        seq = now.replace(":", "").replace("-", "").replace(".", "")[:14]
        report_id = f"eval-{project_id}-{seq}"

        project_metrics = self.evaluate_project(
            project_id, shared_state, project_dir, task_board_data
        )
        ...
        total_violations = sum(
            len(shared_state.get("_meta", {}).get("governance_violations", []))
            for _ in [1]
        )
        ...
        return EvaluationReport(
            ...
            recommendations={
                ...
                "priority_ranking": sorted(
                    [m.metric for m in project_metrics if m.score < 70.0],
                    key=lambda x: next(
                        m.score for m in project_metrics if m.metric == x
                    ),
                ),
                "suggested_actions": (
