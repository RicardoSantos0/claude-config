...
class GraphMemory:
    ...
    def get_related(
        ...
        """
        Return nodes related to node_id, optionally filtered by rel_type.
        """
        neighbors = self.store.neighbors(node_id, depth=depth)
        if rel_type is None:
            return neighbors

        # Filter by edge rel_type — only available with networkx
        ...
        for n in neighbors:
            nid = n.get("node_id", "")
            # Check if any edge between node_id and nid has the requested rel_type
    ...
    def write_episode(
        ...
        Episode types:
          ...
          - "decision"  → creates decision node + edge from deciding agent
          - "artifact"  → creates artifact node + edge from producing agent
          - "phase"     → creates phase node + edge from project
          - "finding"   → creates finding node + edge to related entity
          - "proposal"  → creates proposal node + edge from trainer
    ...
    def _bound_to_tokens(self, lines: list[str], max_tokens: int) -> list[str]:
        """Return as many lines as fit within max_tokens."""
        result = []
        total = 0
        for line in lines:
...
# ---------------------------------------------------------------------------
# EpisodeWriter — called by handoff_engine
# ---------------------------------------------------------------------------
...
class EpisodeWriter:
    ...
    def record_handoff(self, handoff: dict) -> str:
        """Record a handoff as a graph episode. Returns episode_id."""
