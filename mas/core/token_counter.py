"""
Token Counter
Estimates token counts for strings and message lists.

Default backend: char/4 heuristic (fast, no dependencies, ~10% error for English text).
Optional backend: tiktoken (accurate, requires `pip install tiktoken`).

Usage:
    from core.token_counter import TokenCounter
    tc = TokenCounter()
    n = tc.count("hello world")          # 3
    n = tc.count_messages([{"role": "user", "content": "hi"}])

    # Force tiktoken if installed
    tc = TokenCounter(backend="tiktoken", model="claude-sonnet-4-6")

CLI:
    uv run python mas/core/token_counter.py "some text here"
    uv run python mas/core/token_counter.py --backend tiktoken "some text"
"""
