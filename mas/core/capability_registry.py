...
Match scoring (from expert answers):
...
Usage as library:
    ...
    results = registry.search(["reporting", "data-visualization", "salesforce"])

Usage as CLI:
    uv run python core/capability_registry.py search --tags "reporting,dashboard"
    uv run python core/capability_registry.py register --entry-json '{...}'
    uv run python core/capability_registry.py retire --agent-id foo --reason "replaced"
    uv run python core/capability_registry.py gap-cert --project-id proj-001 \\
        --requested-by product_manager_agent \\
        --need "Salesforce reporting integration" \\
        --tags "reporting,salesforce,dashboard"
    uv run python core/capability_registry.py show --agent-id foo
"""
...
class CapabilityRegistry:
    ...
    def produce_gap_certificate(
        ...
        # Determine if a partial match could be parameterized
        ...
        # Build sequence for certificate ID
        data = self.load_registry()
        counts = data.get("counts", {})
        # Use a simple counter based on version history length
